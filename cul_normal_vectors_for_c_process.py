...
def main(data_1, num):
    ...
    for i in range(0, num):
        ...
        rad = np.pi / 2.0
        rot = -1.0 * np.array([[np.cos(rad), -np.sin(rad)],
                    [np.sin(rad), np.cos(rad)]])
        normal_gamma = np.dot(rot, gamma_vec)
        unit_normal_gamma = normal_gamma / np.linalg.norm(normal_gamma)

        # out put of normal_vector to file

        with open('normal.txt', 'a') as f_handle:
            np.savetxt(f_handle, [unit_normal_gamma], delimiter=" ")
        print(unit_normal_gamma.dtype)
...
if __name__ == '__main__':
    num_str = linecache.getline('data_1.txt', 1)
    num = int(num_str)
    print(num)

    data_1 = np.loadtxt("data_1.txt",       # 読み込みたいファイルのパス
